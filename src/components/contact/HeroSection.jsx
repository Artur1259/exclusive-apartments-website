    ...
    <div className="max-w-[380px] sm:max-w-[620px] md:max-w-[750px] lg:max-w-[980px] xl:max-w-[1152px] mx-auto pb-10 ">
      <div className="w-full h-[90vh] bg-[url('/images/hero.avif')] bg-no-repeat bg-cover rounded-[4rem] ">
        <div className="lg:w-[60%] mf:w-[70%] w-full h-full flex flex-col items-start justify-center gap-4 px-10">
          ...
          <span className="text-2xl py-3">{t("contact.hero.subtitle")}</span>
          <form
            onSubmit={handleSubmit}
            className="md:w-[70%] lg:w-[90%] w-full md:flex md:flex-row flex flex-col gap-4 bg-gray-100 py-5 px-5 rounded-2xl relative"
          >
            <input
              type="text"
              value={email}
              ...
              placeholder={t("contact.hero.placeholder")}
              className="w-full px-4 py-3 rounded-full bg-white text-gray-600 "
            />
            {emailError && <div className="absolute left-6 -bottom-1 text-red-500 ">{emailError}</div>}
            <button
              type="submit"
